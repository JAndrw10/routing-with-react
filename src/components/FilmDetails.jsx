...
export default function FilmDetails() {
    ...
    useEffect(() => {
        fetch(`https://ghibliapi.herokuapp.com/films/${filmid}`) // fetch the data
            .then(res => res.json()) // convert from json to js
            .then(film => setFilm(film))// set state
                    ...
                    <div key={film.id} className="card my-3" style={{ width: "18rem" }}>
                        <div className="card-body">
                            ...
                            <a href={`https://ghibliapi.herokuapp.com/films/${film?.id}`} className="card-link">
                                <button className="btn btn-sm btn-primary">View JSON Data</button>
                            </a>
                        </div>
                    </div>
                </div>
